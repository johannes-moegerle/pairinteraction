...
void Basis<Derived>::perform_blocks_checks(const std::set<SorterType> &unique_labels) const {
    ...
    for (const auto &label : unique_labels) {
        const std::string &name = get_quantum_number_name(label);
...
template <typename Derived>
const std::string &Basis<Derived>::get_quantum_number_name(SorterType label) {
    auto it = Derived::quantum_number_names.find(label);
    if (it == Derived::quantum_number_names.end()) {
        throw std::invalid_argument(
            "The states of this basis cannot be labeled by the requested sorter type.");
    }
    return it->second;
}
...
void Basis<Derived>::get_indices_of_blocks_without_checks(
    ...
    unique_quantum_number_names.reserve(unique_labels.size());
    for (const auto &label : unique_labels) {
        unique_quantum_number_names.push_back(get_quantum_number_name(label));
    }
